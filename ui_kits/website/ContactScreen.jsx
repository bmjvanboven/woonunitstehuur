...
function WUContact({ onNav }) {
  const NS = window.WoonunitstehuurDesignSystem_c98620;
  const { Button, Input, Select, Textarea, Badge } = NS;
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1.5fr 1fr', gap: 40, alignItems: 'start' }}>
          ...
          <div style={{ background: 'var(--surface-card)', border: '1px solid var(--border-subtle)', borderRadius: 'var(--radius-lg)', padding: 32, boxShadow: 'var(--shadow-sm)' }}>
            {sent ? (
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
                ...
                <Input label="Achternaam" placeholder="Achternaam" />
                <Input label="E-mailadres" type="email" placeholder="naam@voorbeeld.nl" iconLeft={<I.Mail size={17} />} />
                ...
                <div style={{ gridColumn: '1 / -1', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16, flexWrap: 'wrap' }}>
                  ...
                  <Button variant="primary" size="lg" iconRight={<I.Arrow size={18} />} onClick={() => setSent(true)}>Verstuur aanvraag</Button>
                </div>
              </div>
            )}
          </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
            <div style={{ background: 'var(--ink-800)', color: '#fff', borderRadius: 'var(--radius-lg)', padding: 28, display: 'flex', flexDirection: 'column', gap: 20 }}>
              ...
              {[[<I.Phone size={18} />, '085 — 902 6800', 'Ma–vr 08:00–18:00'], [<I.Mail size={18} />, 'info@woonunitstehuur.nl', 'Reactie binnen één werkdag'], [<I.MapPin size={18} />, 'Heel Nederland', 'Plaatsing op locatie']].map((row, i) => (
