// Hero section — "Kaart" layout (bright, airy, rounded photo card).
// The active animation level (see variants.css) adds motion on top of
// this base markup. Exports window.WUHero (+ alias window.WUHeroBase).
function WUHeroBase({ onNav }) {
      ...
      <div style={{ position: 'relative', maxWidth: 'var(--container-max)', margin: '0 auto', padding: isMobile ? '44px 20px 56px' : '72px 24px', display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '0.92fr 1.08fr', gap: isMobile ? 40 : 56, alignItems: 'center' }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 22 }}>
          ...
          <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', marginTop: 4 }}>
            <Button variant="primary" size="lg" fullWidth={isMobile} iconRight={<I.Arrow size={18} />} onClick={() => onNav('aanbod')}>Bekijk het aanbod</Button>
            <Button variant="outline" size="lg" fullWidth={isMobile} onClick={() => onNav('contact')}>Vraag offerte aan</Button>
          </div>
        ...
        <div style={{ position: 'relative' }}>
          ...
          {/* overlapping availability + price card */}
...
window.WUHeroBase = WUHeroBase;
window.WUHero = WUHeroBase;
