...
function WUHome({ onNav }) {
  const NS = window.WoonunitstehuurDesignSystem_c98620;
  const { Button, UnitCard, Badge } = NS;
  const I = window.WUIcons;
  const { UNITS, USPS, STEPS } = window.WUData;

  const Eyebrow = ({ children, light }) => (
    <div style={{ display: 'flex', alignItems: 'center', gap: 10, fontWeight: 700, fontSize: 13, letterSpacing: '0.12em', textTransform: 'uppercase', color: light ? 'var(--gold-400)' : 'var(--gold-700)' }}>
  ...
  return (
    <div>
      {/* HERO (animation level applied via variants.css) */}
      <window.WUHero onNav={onNav} />
          ...
          {USPS.map((u, i) => {
            ...
            return (
              <div key={i} style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                <span style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', width: 52, height: 52, borderRadius: 'var(--radius-md)', background: 'var(--gold-50)', color: 'var(--gold-700)' }}><Ico size={24} /></span>
      ...
      <section style={{ background: 'var(--surface-page)', borderTop: '1px solid var(--border-subtle)', borderBottom: '1px solid var(--border-subtle)' }}>
        <div style={{ maxWidth: 'var(--container-max)', margin: '0 auto', padding: '72px 24px' }}>
          <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', marginBottom: 32, gap: 24, flexWrap: 'wrap' }}>
            ...
            <Button variant="ghost" iconRight={<I.Arrow size={17} />} onClick={() => onNav('aanbod')}>Bekijk alle units</Button>
          </div>
      ...
      <section style={{ background: 'var(--gold-500)' }}>
        <div style={{ maxWidth: 'var(--container-max)', margin: '0 auto', padding: '56px 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 28, flexWrap: 'wrap' }}>
          ...
          <div style={{ display: 'flex', gap: 14, flexWrap: 'wrap' }}>
            ...
            <Button variant="outline" size="lg" onClick={() => onNav('contact')} style={{ borderColor: 'var(--ink-800)', color: 'var(--ink-900)' }}>Offerte aanvragen</Button>
          </div>
        </div>
      </section>
    </div>
